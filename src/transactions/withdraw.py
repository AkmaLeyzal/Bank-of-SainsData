"""
withdraw.py — Handler logika Withdraw (Tarik Tunai).
"""
...
class WithdrawHandler:
    """Mengelola alur transaksi Withdraw."""
    ...
    def open_form(self):
        """Buka form input nominal withdraw."""
        self._frame_w = tk.Frame(self._parent)
        self._frame_w.place(x=420, y=200)

        photo = ImageCache.get("images/Frame 15 (5).png", (450, 300))
        tk.Label(self._frame_w, image=photo).pack()

        tk.Button(self._frame_w, text="X", font=('Helvetica', 13, 'bold'),
                  border=0, bg="#e8bf83", fg="white",
                  activebackground="#e8bf83", activeforeground="white",
                  command=self._frame_w.destroy).place(x=409, y=21)

        self._nominal_entry = tk.Entry(self._frame_w, width=17, border=0,
                                       bg="#ffffff", font=('Helvetica', 20))
        self._nominal_entry.place(x=94, y=108)

        tk.Button(self._frame_w, text="WITHDRAW", width=18,
    ...
    def _show_confirmation(self, my_norek, nominal, angka_tf):
        ...
        tk.Label(frame_k, text='Withdraw', width=24, border=0, anchor='w',
    ...
    def _process_pin(self, pin: str, my_norek, nominal, angka_tf, pad):
        ...
        if pin_row is None:
            ...
            log_pin_failed(username, "withdraw")
        ...
        self._db.insert_transaction(login_name, "Withdrawal", nominal)
        log_transaction(login_name, "Withdrawal", nominal)

        if self._state.get('click_count', 0) % 2 == 1:
            self._state['show_balance_callback']()

        no_resi = generate_resi("W")
        cur_time, cur_date = current_time_date()
        save_receipt("withdraw", angka_tf, no_resi, login_name, new_balance)

        if self._frame_w and self._frame_w.winfo_exists():
            self._frame_w.destroy()
    ...
    def _show_receipt(self, name, my_norek, angka_tf, cur_time, cur_date, no_resi):
        ...
        items = [
            (name, 75), (my_norek, 97), ('Tarik Tunai', 119),
